...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CadexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CadexCore
    // Mac: ~/Library/Application Support/CadexCore
    // Unix: ~/.cadexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CadexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CadexCore";
#else
    // Unix
    return pathRet / ".cadexcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cadex.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cadex.conf
