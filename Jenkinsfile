...
// which are part of the Cadex repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="cadex-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /cadex-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "cadexproject-cadex/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from cadexproject-cadex/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'cadexproject-cadex/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from cadexproject-cadex/develop"
            }
          }

          if (hasCache) {
            sh "cd /cadex-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /cadex-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /cadex-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /cadex-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /cadex-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /cadex-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /cadex-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
