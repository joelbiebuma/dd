...
*Setup instructions for a Gitian build of Cadex Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cadex
...
- [Building Cadex Core](#building-cadex-core)
...
Clone the git repositories for Cadex Core and Gitian.
...
in the Cadex Core repository under 'Fetch and create inputs' to install sources which require
...
To build Cadex Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Cadex Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cadex/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cadexproject/cadex
...
For example:
```bash
URL=https://github.com/crowning-/cadex.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cadex=${COMMIT} --url cadex=${URL} ../cadex/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cadex=${COMMIT} --url cadex=${URL} ../cadex/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cadex=${COMMIT} --url cadex=${URL} ../cadex/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cadex git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cadex/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/cadex
SIGPATH=/some/root/path/cadex-detached-sigs

./bin/gbuild --url cadex=${BTCPATH},signature=${SIGPATH} ../cadex/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cadex-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cadex-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cadex-osx-build.assert
```
...
[cadex/gitian.sigs](https://github.com/cadexproject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
